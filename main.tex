...
Overall, the Phase 2 version of LexiClear represents an extension of the original document-analysis pipeline into a more complete interactive system. The core workflow of PDF processing, OCR, preprocessing, semantic representation, vector retrieval, and RAG remains the technical foundation. On top of this foundation, the current system introduces structured document analysis, jurisdiction grounding, interactive Ask LexiClear questioning, multilingual output in English, Hindi and Kannada, translation, and text-to-speech interaction. These additions broaden the ways in which users can access and interact with complex document information while maintaining the central design principle of grounding document-specific responses in retrieved source content.
...
\section{Dataset}
LexiClear is designed to process legal and administrative PDF documents rather than depending on a single fixed dataset. Such documents may include rental agreements and other formal documents containing clauses, conditions, obligations, rights, responsibilities, and potential risks.

The documents used during development and functional testing provide examples of the type of content for which the system is intended. A rental agreement was used during testing of the document-analysis workflow, allowing the system to process document content and generate structured information from it.
...
The processed document chunks and their associated metadata form the searchable knowledge representation used by the retrieval system. Metadata can include information such as the source document, page number, section, and other available document-location details.

The dataset used for development is therefore representative of the intended document-processing scenario rather than a formally curated benchmark dataset. An expert-labelled dataset containing ground-truth summaries, obligations, risks, rights, and question-answer pairs has not been established at this stage. Consequently, the system is not evaluated using claims such as classification accuracy, precision, recall, or F1-score.
...
The developed system supports both digital and scanned PDF documents through direct text extraction and Optical Character Recognition (OCR). The extracted content is preprocessed, divided into meaningful chunks, converted into semantic embeddings, and stored for retrieval. When document analysis or a user question is submitted, relevant document content is retrieved and supplied to the RAG-based generation component. This allows responses to be generated using information associated with the uploaded document rather than relying only on general language-model knowledge.
