...
The Ask LexiClear functionality was evaluated using document-specific question-answer data prepared from the legal documents used during functional testing. The evaluation records the question, expected answer, AI-generated answer, semantic similarity, BERTScore Precision, BERTScore Recall, BERTScore F1, and QA correctness.

Figure~\ref{fig:qa_evaluation} shows the question-level evaluation results generated during testing. The results provide a direct comparison between the expected answers and the responses generated by LexiClear. The evaluation also includes semantic similarity and BERTScore-based measures to assess the similarity between the generated and expected answers.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{qa_evaluation_results.png}
    \caption{Question-Level QA Evaluation Results}
    \label{fig:qa_evaluation}
\end{figure}

Figure~\ref{fig:qa_evaluation} describes the detailed evaluation results for the tested question-answer pairs. Each row represents one question and contains the corresponding expected answer, AI-generated answer, semantic similarity score, BERTScore Precision, BERTScore Recall, BERTScore F1, and QA correctness result.

The aggregate evaluation metrics obtained from the tested questions are presented in Table~\ref{tab:qa_metrics}.

\begin{table}[h]
\centering
\caption{Aggregate Question-Answer Evaluation Metrics}
...
\hline
Average Semantic Similarity & 0.5564 \\
\hline
Average BERTScore Precision & 0.8736 \\
\hline
Average BERTScore Recall & 0.8883 \\
\hline
Average BERTScore F1 & 0.8807 \\
\hline
QA Accuracy & 0.1600 \\
\hline
\end{tabular}
...
The evaluation was performed on 25 question-answer pairs. The average semantic similarity score was 0.5564, while the average BERTScore Precision, Recall, and F1 scores were 0.8736, 0.8883, and 0.8807, respectively. The recorded QA accuracy was 0.1600.

These results provide a quantitative view of the responses generated by the Ask LexiClear functionality for the tested questions. The semantic similarity and BERTScore measures indicate the degree of textual similarity between the expected and generated answers, while the QA accuracy represents the correctness result recorded by the evaluation procedure. The results are therefore reported as evaluation measurements for the tested question set and should not be interpreted as a general measure of legal interpretation accuracy.

The evaluation was conducted on a limited set of document-specific questions and does not constitute a large expert-annotated legal benchmark. Therefore, the reported values represent the current functional evaluation results and should be interpreted within the scope of the tested questions and documents.
